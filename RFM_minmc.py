...
def estimate_mc(magnitudes, bin_size=0.1, m_min=None, m_max=None, mi_step=0.1, max_mc=None, min_mc=None):
    """
    ...
    if min_mc is not None:
        lower_limit = max(m_min, min_mc)
    else:
        lower_limit = m_min

    mi_candidates = np.arange(lower_limit, m_max - 0.5, mi_step)
    R_values = {}
...
if __name__ == "__main__":
    ...
    for year in range(1960, 2022):
        ...
        mc, a, b, R_values, bins, O_full = estimate_mc(magnitudes, bin_size=0.1, mi_step=0.1, min_mc=5.0)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('Mc_vs_Year_min_mc.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("График Mc по годам сохранён как Mc_vs_Year_min_mc.png")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('b_vs_Year_min_mc.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("График b-values по годам сохранён как b_vs_Year_min_mc.png")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('a_vs_Year_min_mc.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("График a-values по годам сохранён как a_vs_Year_min_mc.png")
