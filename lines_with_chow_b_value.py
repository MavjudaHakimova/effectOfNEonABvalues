...
query = 'SELECT year, b FROM "earthquakes_gr_low"."values_results_rfm"'
...
pooled_model = sm.OLS(df['b'], Xp).fit()
RSS_pooled = np.sum(pooled_model.resid**2)

results = []

break_years = range(1970, 1998)
...
print(f"\nДо {best_year}:")
print(f"b(t) = {a_before:.6f} + {b_before:.6f} * t")

print(f"\nПосле {best_year}:")
print(f"b(t) = {a_after:.6f} + {b_after:.6f} * t")

print("\nLaTeX запись:")
print(f"До {best_year}: $b(t) = {a_before:.4f} + {b_before:.6f}t$")
print(f"После {best_year}: $b(t) = {a_after:.4f} + {b_after:.6f}t$")
...
plt.tight_layout()
plt.savefig('chow_min_mc.png', dpi=300, bbox_inches='tight')
print("График сохранен как 'chow_min_mc.png'")
plt.show()
...
plt.plot(df.year, df.b,'o-', label="Данные")
...
plt.grid()
plt.tight_layout()
plt.savefig(f'chow_best_{best_year}_min_mc.png', dpi=300, bbox_inches='tight')
print(f"График регрессии сохранен как 'chow_best_{best_year}_min_mc.png'")
plt.show()
